...
func (s *server) handlerShortenLink(w http.ResponseWriter, r *http.Request) {
	...
	//s.logger.Info(fmt.Sprintf("Shortening URL: %s\n", longURL))
	...
	if err != nil {
		httpError(r.Context(), w, fmt.Errorf("failed to shorten URL"), http.StatusInternalServerError)
		return
	}
	s.logger.Info(
		"Generated Short URL",
...
func (s *server) handlerListURLs(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httpError(r.Context(), w, fmt.Errorf("failed to list URLs"), http.StatusInternalServerError)
		return
	}
